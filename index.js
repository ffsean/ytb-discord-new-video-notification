...
async function checkYouTubeChannels() {
  ...
  // 關鍵字篩選設定
  const keywords = ["弾き語り", "昭和", "歌枠", "歌ってみた", "cover"];
  const titleLower = latestVideo.title.toLowerCase();
  const matched = keywords.some(k => titleLower.includes(k.toLowerCase()));

  // 如果標題不含任何關鍵字，就跳過通知
  if (!matched) {
    console.log(`❌ 跳過影片（不含關鍵字）：${latestVideo.title}`);
    // 雖然跳過，但仍更新日期，避免重複通知
  ...
  const messageText = `🎬 新影片（含關鍵字）來自 ${channelData.title}：\n${latestVideo.title}\n🔗 ${latestVideo.link}`;
  await discordChannel.send(messageText);

  console.log(`✅ 通知：${latestVideo.title}`);
}
// index.js: Main bot file
